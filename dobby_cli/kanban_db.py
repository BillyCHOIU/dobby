...
from dobby_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    if resolved in _INITIALIZED_PATHS:
        ...
        try:
            ...
            with _INIT_LOCK:
                from dobby_state import apply_wal_with_fallback
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from dobby_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    if project_id:
        try:
            from dobby_cli import projects_db as _pdb
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    if not rows:
        return False
    try:
        from dobby_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from dobby_cli.profiles import profile_exists as _pe
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from dobby_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _resolve_worker_cli_toolsets(hermes_home: Optional[str]) -> Optional[list[str]]:
    ...
    if not hermes_home:
        return None
    try:
        from dobby_constants import reset_hermes_home_override, set_hermes_home_override
        from dobby_cli.config import load_config
