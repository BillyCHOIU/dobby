...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from dobby_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        from dobby_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from dobby_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from dobby_cli.config import load_config
            from dobby_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from dobby_cli.providers import resolve_user_provider as _ruser
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from dobby_cli.models import normalize_opencode_base_url
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from dobby_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from dobby_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from dobby_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from dobby_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        if not has_creds:
            try:
                from dobby_cli.auth import _load_auth_store
    ...
    from dobby_cli.providers import HERMES_OVERLAYS
    from dobby_cli.auth import PROVIDER_REGISTRY as _auth_registry
    ...
    for pid, overlay in HERMES_OVERLAYS.items():
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            try:
                ...
                from dobby_cli.auth import get_provider_auth_state as _nous_state
    ...
    # with `hermes model`).
    try:
        from dobby_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from dobby_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    """
    from dobby_cli.models import fetch_openrouter_models
