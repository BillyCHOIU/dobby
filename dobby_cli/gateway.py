...
def find_profile_gateway_processes(
    ...
    try:
        from gateway.status import get_running_pid
        from dobby_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    from dobby_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from dobby_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # base interpreter needs to import dobby_cli.  Empty on POSIX, where
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from dobby_cli.service_manager import detect_service_manager, get_service_manager
...
def _gateway_list() -> None:
    ...
    """
    try:
        from dobby_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
from dobby_constants import is_container, is_termux, is_wsl
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from dobby_cli.telegram_managed_bot import (
...
def _builtin_setup_fn(key: str):
    ...
    """
    from dobby_cli import setup as _s
