...
from dobby_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from dobby_cli.memory_providers import (
    MemoryProvider,
    ProviderField,
    get_memory_provider,
)
...
def _warm_gateway_module() -> None:
    try:
        import dobby_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from dobby_cli.gateway import _get_restart_drain_timeout
...
from dobby_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# ---------------------------------------------------------------------------
from dobby_cli.dashboard_auth.public_paths import (
...
async def _dashboard_auth_gate(request: Request, call_next):
    from dobby_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from dobby_cli.dashboard_auth.token_auth import token_auth_middleware
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    """
    from dobby_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from dobby_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from dobby_cli.models import _AGGREGATOR_PROVIDERS
...
def _dashboard_local_update_managed_externally() -> bool:
    ...
    if _default_hermes_root_is_opt_data():
        return True
    try:
        from dobby_constants import is_container
...
from dobby_cli import web_git as _web_git  # noqa: E402
...
async def get_status(profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            from dobby_cli.dashboard_auth import list_providers as _list_providers
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from dobby_cli.auth import get_nous_auth_status
    ...
    features = []
    try:
        from dobby_cli.nous_subscription import get_nous_subscription_features
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from dobby_cli.debug import build_debug_share
...
def get_profiles_sessions(
    ...
    from dobby_state import SessionDB
    from dobby_cli import profiles as profiles_mod
...
def get_recommended_default_model(provider: str = ""):
    ...
    if slug == "nous":
        try:
            ...
            from dobby_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from dobby_cli.moa_config import normalize_moa_config
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        if provider.strip().lower() == "nous":
            try:
                from dobby_cli.nous_subscription import apply_nous_managed_defaults
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from dobby_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    """
    try:
        from dobby_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from dobby_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        pass
    try:
        from dobby_cli.config import get_env_value
    except ImportError:
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from dobby_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        if provider_id == "anthropic":
            ...
            try:
                from dobby_cli.auth import clear_provider_auth
        ...
        try:
            from dobby_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
async def _start_device_code_flow(
    ...
    if provider_id == "xai-oauth":
        from dobby_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from dobby_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from dobby_cli.auth import _save_codex_tokens
...
async def get_logs(
    ...
    from dobby_cli.logs import _read_tail, LOG_FILES
    ...
    try:
        from dobby_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
async def list_mcp_servers(profile: Optional[str] = None):
    from dobby_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from dobby_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from dobby_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from dobby_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from dobby_cli.mcp_config import (
        _get_mcp_servers,
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    """
    try:
        from dobby_cli import mcp_catalog
    except Exception as exc:
...
async def list_webhooks():
    import dobby_cli.webhook as wh
...
async def list_credential_pool():
    ...
    from dobby_cli.auth import read_credential_pool
...
async def list_hooks():
    ...
    form can offer them.
    """
    from dobby_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from dobby_cli.plugins import VALID_HOOKS
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from dobby_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    """
    from dobby_constants import set_hermes_home_override, reset_hermes_home_override
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    """
    from dobby_constants import set_hermes_home_override, reset_hermes_home_override
    from dobby_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    """
    from dobby_constants import set_hermes_home_override, reset_hermes_home_override
    from dobby_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    _resolve_profile_dir(name)
    try:
        from dobby_cli import profile_describer
...
async def get_skills(profile: Optional[str] = None):
    ...
    from dobby_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    from dobby_cli.tools_config import (
...
if sys.platform.startswith("win"):
    try:
        from dobby_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from dobby_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from dobby_cli.dashboard_auth.audit import AuditEvent, audit_log
        from dobby_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    """
    from dobby_cli.main import PROJECT_ROOT, _make_tui_argv
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from dobby_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from dobby_cli.console_engine import HermesConsoleEngine
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    """
    from dobby_cli.dashboard_auth.prefix import normalise_prefix
...
def _discover_dashboard_plugins() -> list:
    ...
    from dobby_cli.plugins import get_bundled_plugins_dir
...
async def get_dashboard_plugins():
    ...
    try:
        from dobby_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from dobby_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from dobby_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from dobby_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from dobby_cli.plugins_cmd import dashboard_remove_user_plugin
...
from dobby_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def start_server(
    ...
    try:
        from dobby_cli.nous_auth_keepalive import start_nous_auth_keepalive
