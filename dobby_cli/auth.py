...
from dobby_cli.config import get_hermes_home, get_config_path, read_raw_config
from dobby_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
try:  # Version tag for the Codex token-endpoint User-Agent; fall back if unavailable.
    from dobby_cli import __version__ as _HERMES_CLI_VERSION
...
def get_anthropic_key() -> str:
    ...
    """
    from dobby_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from dobby_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    try:
        from dobby_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
...
def _prompt_model_selection(
    ...
    """
    from dobby_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from dobby_cli.config import save_config, load_config
