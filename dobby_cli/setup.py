...
from dobby_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from dobby_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from dobby_cli.curses_ui import curses_checklist
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from dobby_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from dobby_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    from dobby_cli.main import select_provider_and_model
    try:
...
def _setup_telegram_auto_result():
    ...
    try:
        from dobby_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None
...
def _setup_qqbot():
    ...
    from dobby_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from dobby_cli.gateway import _all_platforms, _platform_status, _configure_platform
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from dobby_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from dobby_cli.config
    ...
    if section_key == "model":
        ...
        from dobby_cli.gateway import _all_platforms, _platform_status
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    try:
        from dobby_cli.main import _model_flow_nous
...
def run_setup_wizard(args):
    ...
    """
    from dobby_cli.config import is_managed, managed_error
    if is_managed():
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    try:
        ...
        from dobby_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
