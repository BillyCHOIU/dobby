...
def _apply_user_default_headers(headers: dict | None) -> dict | None:
    ...
    """
    try:
        from dobby_cli.config import cfg_get, load_config
        _cfg = load_config()
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "nous":
        ...
        from dobby_cli.auth import _nous_inference_env_override
...
def _nous_api_key(provider: dict) -> str:
    ...
    from dobby_cli.auth import _nous_invoke_jwt_is_usable
...
def _resolve_nous_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from dobby_cli.auth import _agent_key_is_usable
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    try:
        from dobby_cli.auth import resolve_nous_runtime_credentials
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    try:
        from dobby_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from dobby_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from dobby_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from dobby_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from dobby_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from dobby_cli.models import get_nous_recommended_aux_model
...
def _try_azure_foundry(
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from dobby_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _nous_portal_account_has_fresh_paid_access() -> bool:
    ...
    try:
        from dobby_cli.nous_account import get_nous_portal_account_info
...
def _resolve_auto(
    ...
    if main_provider == "moa":
        try:
            from dobby_cli.config import load_config
            from dobby_cli.moa_config import resolve_moa_preset
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from dobby_cli.copilot_auth import copilot_request_headers
...
def resolve_provider_client(
    ...
    try:
        from dobby_cli.runtime_provider import _get_named_custom_provider
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from dobby_cli.models import _should_use_copilot_responses_api
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from dobby_cli.providers import get_provider
