...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from dobby_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from dobby_cli.plugins import invoke_hook
...
def _max_live_sessions() -> int:
    try:
        from dobby_cli.active_sessions import coerce_max_concurrent_sessions
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from dobby_cli.runtime_provider import canonical_custom_identity
...
def resolve_skin() -> dict:
    try:
        from dobby_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    try:
        from dobby_cli.models import detect_static_provider_for_model
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from dobby_cli.runtime_provider import (
...
def _load_enabled_toolsets() -> list[str] | None:
    ...
    if explicit and validate_toolset is not None:
        ...
        try:
            ...
            from dobby_cli.tools_config import _parse_enabled_flag
...
def _apply_model_switch(
    ...
    *,
    ...
    from dobby_cli.runtime_provider import resolve_runtime_provider
    ...
    custom_provs = None
    try:
        from dobby_cli.config import get_compatible_custom_providers, load_config
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from dobby_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from dobby_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from dobby_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from dobby_cli.config import load_config as _load_full_cfg
...
def _resolve_runtime_with_fallback(
    ...
    """
    from dobby_cli.auth import AuthError
    from dobby_cli.runtime_provider import resolve_runtime_provider
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from dobby_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from dobby_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from dobby_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from dobby_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from dobby_cli.pets import set_pet_scale
...
def _(rid, params: dict) -> dict:
    ...
    """
    from dobby_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from dobby_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from dobby_cli.nous_billing import BillingError, patch_auto_top_up
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from dobby_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from dobby_cli.voice import speak_text
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from dobby_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from dobby_cli.model_switch import parse_model_flags
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from dobby_cli.models import resolve_fast_mode_overrides
...
def _projects_payload(conn) -> dict:
    from dobby_cli import projects_db as pdb
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from dobby_cli.models import list_available_providers, normalize_provider
...
def _(rid, params: dict) -> dict:
    try:
        from dobby_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from dobby_cli.runtime_provider import resolve_runtime_provider
        from dobby_cli.auth import has_usable_secret
        from dobby_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from dobby_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from dobby_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from dobby_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from dobby_cli.config import is_managed, save_env_value
        from dobby_cli.inventory import build_models_payload, load_picker_context
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from dobby_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from dobby_cli.config import remove_env_value
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        if not enabled:
            ...
            try:
                from dobby_cli.voice import stop_continuous
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from dobby_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from dobby_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from dobby_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from dobby_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from dobby_cli.skills_hub import do_install
        ...
        if action == "browse":
            from dobby_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from dobby_cli.skills_hub import inspect_skill
