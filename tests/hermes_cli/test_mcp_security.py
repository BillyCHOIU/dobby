...
def _isolate_config(tmp_path, monkeypatch):
    ...
    import dobby_cli.config as config_mod
...
def test_save_rejects_hermes_0day_persistence_entry():
    ...
    from dobby_cli.mcp_config import _save_mcp_server
...
def test_probe_rejects_dangerous_entry_before_connect(monkeypatch):
    from dobby_cli.mcp_config import _probe_single_server
...
def test_migration_disables_existing_dangerous_entry(tmp_path):
    ...
    from dobby_cli.config import load_config, migrate_config
...
def test_dashboard_mcp_add_rejects_dangerous_entry():
    ...
    from dobby_cli.web_server import _SESSION_HEADER_NAME, _SESSION_TOKEN, app
...
def test_profile_mcp_write_skips_dangerous_entry(tmp_path):
    from dobby_cli.config import load_config
    from dobby_cli.web_server import MCPServerCreate, _write_profile_mcp_servers
